...
class CronService:
    ...
    - File-based (jobs.json) - legacy, used when no db is provided
    - SQLite - used when a Database instance is provided
    """
    ...
    def _get_sqlite_store(self):
        ...
        if self._sqlite_store is None and self.db is not None:
            from comobot.cron.sqlite_store import SQLiteCronStore

            self._sqlite_store = SQLiteCronStore(self.db)
    ...
    def _load_store_file(self) -> CronStore:
        """Load jobs from disk (file-based). Reloads if file was modified externally."""
    ...
    def _save_store_file(self) -> None:
        """Save jobs to disk (file-based)."""
    ...
    async def _load_store(self) -> CronStore:
        """Load jobs from the active backend."""
        if self._use_sqlite:
            store = await self._get_sqlite_store().load()
            self._store = store
            return store
        return self._load_store_file()

    async def _save_store(self) -> None:
        """Save jobs to the active backend."""
        if not self._store:
            return
        if self._use_sqlite:
            await self._get_sqlite_store().save(self._store)
        else:
            self._save_store_file()

    async def start(self) -> None:
        """Start the cron service."""
        self._running = True
        # Force fresh load on start (bypass mtime cache)
        self._store = None
        ...
        self._arm_timer()
        logger.info(
            "Cron service started with {} jobs (backend: {})",
            len(self._store.jobs if self._store else []),
            "sqlite" if self._use_sqlite else "file",
        )
    ...
    async def add_job(
        ...
        if self._use_sqlite:
            new_id = await self._get_sqlite_store().add_job(job)
            job.id = new_id
        else:
    ...
    async def remove_job(self, job_id: str) -> bool:
        ...
        if self._use_sqlite:
            removed = await self._get_sqlite_store().remove_job(job_id)
        else:
    ...
    async def enable_job(self, job_id: str, enabled: bool = True) -> CronJob | None:
        ...
        for job in store.jobs:
            if job.id == job_id:
                ...
                if self._use_sqlite:
                    await self._get_sqlite_store().update_job(job)
                else:
                    self._save_store_file()
    ...
    def status(self) -> dict:
        """Get service status (sync, uses cached store)."""
