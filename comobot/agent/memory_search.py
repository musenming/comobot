...
class MemorySearchEngine:
    ...
    def _discover_files(self) -> list[tuple[str, Path]]:
        ...
        # Session transcripts (generated by SessionIndexer)
        transcript_dir = self.workspace / ".session_index" / "transcripts"
        if transcript_dir.exists():
            for p in sorted(transcript_dir.glob("*.md")):
                if not p.name.startswith("."):
                    files.append((f".session_index/transcripts/{p.name}", p))
