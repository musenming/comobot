...
class SkillsLoader:
    ...
    def _parse_skill_metadata(self, raw: str) -> dict:
        """Parse skill metadata JSON from frontmatter (supports comobot and openclaw keys)."""
        try:
            data = json.loads(raw)
            if isinstance(data, dict):
                return data.get("comobot", data.get("nanobot", data.get("openclaw", {})))
            return {}
    ...
    def _get_skill_meta(self, name: str) -> dict:
        """Get comobot metadata for a skill (cached in frontmatter)."""
